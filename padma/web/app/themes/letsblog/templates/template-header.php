...
global $letsblog_hide_title;
if($letsblog_hide_title == 1)
{
	$page_show_title = 1;
}
...
?>
<div id="page_caption" class="<?php if(!empty($pp_page_bg)) { ?>hasbg parallax <?php } ?> <?php if(!empty($letsblog_topbar)) { ?>withtopbar<?php } ?> <?php if(!empty($lestblog_screen_class)) { ?>split<?php } ?>">
	<?php if(!empty($pp_page_bg)) { ?>
		<div id="bg_regular" style="background-image:url(<?php echo esc_url($pp_page_bg); ?>);"></div>
	<?php } ?>
	<?php
	    if(!empty($tg_page_title_img_blur) && !empty($pp_page_bg) && $lestblog_screen_class != 'split')
	    {
	?>
	...
	<div class="page_title_wrapper">
		<div class="page_title_inner">
			<h1 <?php if(!empty($pp_page_bg) && !empty($letsblog_topbar)) { ?>class ="withtopbar"<?php } ?>><?php echo esc_html($page_title); ?></h1>
			<?php
...
<!-- Begin content -->
<?php
global $lestblog_page_content_class;
?>
<div id="page_content_wrapper" class="<?php if(!empty($pp_page_bg)) { ?>hasbg <?php } ?><?php if(!empty($pp_page_bg) && !empty($letsblog_topbar)) { ?>withtopbar <?php } ?><?php if(!empty($lestblog_page_content_class)) { echo esc_attr($lestblog_page_content_class); } ?>">