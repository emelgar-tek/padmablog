...
<?php
	$theblogger_sidebar_portfolio_category = get_theme_mod('theblogger_setting_sidebar_portfolio_category', 'No');
?>

<div id="main" class="site-main">
	<div class="<?php theblogger_portfolio_page_layout(); ?>">
		<div id="primary" class="content-area <?php if ($theblogger_sidebar_portfolio_category == 'Yes') { echo 'with-sidebar'; } ?>">
			<div id="content" class="site-content" role="main">
				...
				<ul id="filters" class="filters">
					<?php
						$theblogger_queried_category_id = get_queried_object_id();
						
						$theblogger_categories = get_categories(
							array(
								'type'     => 'portfolio',
								'taxonomy' => 'portfolio-category',
								'parent'   => $theblogger_queried_category_id
							)
				...
				<div class="blog-grid-wrap">
					<div class="blog-stream blog-grid blog-small portfolio-grid masonry" data-layout="<?php echo esc_attr($theblogger_portfolio_page_grid_type_layout); ?>" data-item-width="<?php echo esc_attr($theblogger_portfolio_page_post_width); ?>">
						...
						<?php
							$theblogger_queried_category_slug = get_query_var('term');
							
							$theblogger_query = new WP_Query(
								array(
									'post_type'          => 'portfolio',
									'portfolio-category' => $theblogger_queried_category_slug,
									'posts_per_page'     => -1
								)
							);
				...
				<?php
					$theblogger_category_description = category_description();
					
					if (! empty($theblogger_category_description))
					{
						?>
							<article id="post-<?php echo esc_attr($theblogger_queried_category_id); ?>">
								<div class="entry-content">
									<?php
										echo esc_html($theblogger_category_description);
									?>
								</div> <!-- .entry-content -->
		...
		<?php
			if ($theblogger_sidebar_portfolio_category == 'Yes')
			{
				theblogger_sidebar();
			}
		?>
